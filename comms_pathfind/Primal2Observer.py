...
class Primal2Observer(ObservationBuilder):
    ...
    def _get(self, agent_id, all_astar_maps):
        ...
        return state, [dx, dy, mag], message, np.array([time1, time2, time3, time4, time5, time6])

    def get_many(self, handles=None):
        observations = {}
        # all_astar_maps = self.get_astar_map()
        all_astar_maps = np.array([])
        ...
        for h in handles:
            state, vector, message, time = self._get(h, all_astar_maps)
            # observations[h] = [state, vector]
            observations[h] = [state, vector, message]
            times += time
        if self.printTime:
            print(times)
        return observations
    ...
    #             astar_path.extend(get_single_astar_path(distance_map1, start_pos1, self.num_future_steps - len(astar_path)))
    #         # if ENV_DEBUG_MODE:
    #         #     print('(get_astar_map)astar_path', astar_path)
    #
    #         for i in range(self.num_future_steps - len(astar_path)):  # only happen when min_distance not sufficient
    #             if not astar_path:
    #                 astar_path.extend([[self.world.agents[agentID].position]])
    #                 continue
    #             astar_path.extend([astar_path[-1]])  # stay at the last pos
    #         if ENV_DEBUG_MODE:
    #             print('(get_astar_map)astar_path', astar_path)
    #
