...
class World:
    ...
    def reset_world(self):
        ...
        # pick up agents from manual map
    ...
    def init_agents_and_goals(self):
        ...
        if ENV_DEBUG_MODE:
            print('(init_agents_and_goals)self.goals_init_pos', self.goals_init_pos)
            print('(init_agents_and_goals)self.agents_init_pos', self.agents_init_pos)
        self._put_goals(list(range(1, self.num_agents + 1)), self.goals_init_pos)
        self._put_agents(list(range(1, self.num_agents + 1)), self.agents_init_pos)
    ...
    def _put_agents(self, id_list, manual_pos=None):
        ...
        """
        # code from PRIMAL1: maybe not connected in PRIMAL1
        ...
        def _getConnectedRegion(world, x, y):
            '''
            returns a list of tuples of connected squares to the given tile this is memoized with a dict
            '''
            sys.setrecursionlimit(1000000)
            sx, sy = world.shape[0], world.shape[1]
            visited = set()
            ...
            # search the visited boundary's 4-connected neighbor
            while len(work_list) > 0:
                (i, j) = work_list.pop()
                if i < 0 or i >= sx or j < 0 or j >= sy:
                    continue    # out of bounds, return
                if world[i, j] == -1:
                    continue    # crashes
                if(i, j) in visited:
                    continue
        ...
        if manual_pos is None:
            ...
            # print('before', init_poss)

            # set the agents same way as setting the goals in PRIMAL1
            agents_map = np.zeros(self.state.shape).astype(int)
            for id in id_list:
                goal_pos = np.argwhere(self.goals_map == id)
                while True:
                    valid_tiles = _getConnectedRegion(self.state, goal_pos[0][0], goal_pos[0][1])
                    x, y = random.choice(list(valid_tiles))
                    if agents_map[x][y] == 0:
                        agents_map[x][y] = id
                        break
            init_poss = [np.argwhere(agents_map == id)[0] for id in id_list]
            if ENV_DEBUG_MODE:
                print('(_put_agents)agent select finished!')
                print('(_put_agents)agents_map', agents_map)
                print('(_put_agents)init_poss', init_poss)
        else:
    ...
    def _put_goals(self, id_list, manual_pos=None):
        """
        ...
        # function: random choose the new goal, return agent goal dic.
        ...
        if ENV_DEBUG_MODE:
            print('(_put_goals)new_goals', new_goals)

        # select next_goals for agents
        if new_goals is not None:  # recursive breaker
            ...
            for agentID in id_list:
                if self.state[new_goals[agentID][0], new_goals[agentID][1]] >= 0:
                    ...
                    print('self.goals_map', self.goals_map)
                ...
                if refresh_distance_map:
                    self.agents[agentID].distanceMap = getAstarDistanceMap(self.state, self.agents[agentID].position,
                                                                           self.agents[agentID].goal_pos)
            if ENV_DEBUG_MODE:
                print('(_put_goals)self.goals_map', self.goals_map)
                print('(_put_goals)self.next_goals_map: todo future.', None)
                print('(_put_goals)goal select finished!')
            return 1
        else:
            return None
