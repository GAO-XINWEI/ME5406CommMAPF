...
LR_Q                    = 2.e-6  # Last version: 5.e-6 # 8.e-5 / NUM_THREADS # default: 1e-5
ADAPT_LR                = True
...
ENVIRONMENT_SIZE        = (32, 32) if not ENV_DEBUG_MODE else (8, 8)  # the total size of the environment (length of one side) , Starting Point of Curriculum Only
...
NUM_META_AGENTS         = 2 if not RAY_DEBUG_MODE else 2
NUM_IL_META_AGENTS      = 1 if not RAY_DEBUG_MODE else 1

NUM_THREADS             = 14 if not ENV_DEBUG_MODE else 4 # int(multiprocessing.cpu_count() / (2 * NUM_META_AGENTS))
...
# PRIMAL1
joint_blocking          = [{} for _ in range(NUM_META_AGENTS)]
# PRIMALC
joint_meangoal          = [{} for _ in range(NUM_META_AGENTS)]
joint_comms             = [{} for _ in range(NUM_META_AGENTS)]
joint_voting            = [{} for _ in range(NUM_META_AGENTS)]
