...
class ACNet:
    def __init__(self, scope, a_size, trainer,TRAINING,GRID_SIZE,GLOBAL_NET_SCOPE):
        with tf.variable_scope(str(scope)+'/qvalues'):
            ...
            self.policy, self.value, self.state_out, self.state_in, self.state_init, self.blocking, self.on_goal, self.mean_goal, self.message, self.valids = self._build_net(self.myinput,self.goal_pos,RNN_SIZE,TRAINING,a_size)
        if TRAINING:
            ...
            self.mean_goal_loss = - tf.reduce_sum(self.target_mean_goals * tf.log(tf.clip_by_value(self.mean_goal, 1e-10, 1.0)) \
                                      + (1 - self.target_mean_goals) * tf.log(tf.clip_by_value(1 - self.mean_goal, 1e-10, 1.0)))
            self.message_loss = - tf.reduce_sum(self. advantages * tf.log(tf.clip_by_value(self.message, 1e-10, 1.0)))  ## todo: debug
    ...
    def _build_net(self,inputs,goal_pos,RNN_SIZE,TRAINING,a_size):
        ...
        mean_goal    = layers.fully_connected(inputs=self.rnn_out, num_outputs=1, weights_initializer=normalized_columns_initializer(1.0), biases_initializer=None, activation_fn=tf.sigmoid)
